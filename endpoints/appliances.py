...
async def create_appliances(
    create_new_appliance: CreateAppliances, session: AsyncSession = Depends(get_session)
):
    try:
        new_appliance = Appliances(**jsonable_encoder(create_new_appliance))
...
async def update_appliance_by_id(
    appliance_id: UUID,
    create_new_appliance: CreateAppliances,
